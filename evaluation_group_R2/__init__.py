...
class Constants(BaseConstants):
    name_in_url = 'evaluation_group_R2'
...
class Player(BasePlayer):
    ...
    score = models.IntegerField(min=0,
                                max=100,
                                label="请您为该卖家打分：")
    score_R2 = models.FloatField()  # 存储一阶加权分数，根据score直接生成
    average_score_R2 = models.FloatField()  # 买家决策依据，根据历史score_R1生成
    average_score = models.FloatField()  # 计算二阶加权分使用，过程性数据（all_rounds数据，即由最新数据生成）
...
class SellerPage(Page):  # 卖家定价页面+生成卖家均分+卖家历史记录
    ...
    def vars_for_template(player):
        ...
        if player.round_number == 1:
            average_score_R2 = None  # 如果是第一轮，那么score_R=None
        else:
            ...
            for i in (buyerA,buyerB,buyerC):
                for j in i.in_previous_rounds():
                    if j.field_maybe_none('buyer_decision') == str(player.role):
                        previous_score_R2.append(round(j.score_R2,2))  # 如果buyer_decision==当前玩家的role的话，那么在列表里面加上score
            print("SellerPage生成该卖家所有历史二阶分数",previous_score_R2)
            average_score_R2 = None if len(previous_score_R2) == 0 else mean(previous_score_R2)
        player.average_score_R2 = average_score_R2  # 一阶加权均分
        ...
        for i in previous_player:  # 获取每轮买家的决策依据（平均分）
            try:
                previous_score_R2.append(str(round(i.average_score_R2,2)) + "分")
            except:
                previous_score_R2.append("无")
        ...
        data = {"历史均分": previous_score_R2, "成本": previous_quality, "定价": previous_price, "卖出件数": previous_purchase_time,
        ...
        return {
            ...
            "average_score_R2": None if average_score_R2 == None else round(average_score_R2,2),
...
def get_table(sellerA, sellerB, sellerC, player):
    # 生成html表格-----------------------------------------------------------
    sellerA_score = [i.field_maybe_none('average_score_R2') for i in sellerA.in_previous_rounds()]
    sellerB_score = [i.field_maybe_none('average_score_R2') for i in sellerB.in_previous_rounds()]
    sellerC_score = [i.field_maybe_none('average_score_R2') for i in sellerC.in_previous_rounds()]
    previous_average_score_R2 = ["无" if j == None else round(j,2) for j in list(chain.from_iterable(zip(sellerA_score,sellerB_score,sellerC_score)))]
    print(previous_average_score_R2)
    ...
    data = {"":seller_list,
            "卖家均分": previous_average_score_R2,
            "商品价格": previous_price,
...
class BuyerPage(Page):  # 买家评分要参考：价格、评分
    ...
    def vars_for_template(player):
        ...
        scoreR_A = player.group.get_player_by_role(choice_seller["A"]).field_maybe_none('average_score_R2')
        scoreR_B = player.group.get_player_by_role(choice_seller["B"]).field_maybe_none('average_score_R2') # sellerA的price
        scoreR_C = player.group.get_player_by_role(choice_seller["C"]).field_maybe_none('average_score_R2')
...
def average_score(the_seller,buyerA,buyerB,buyerC):
    score_list=[]
    for the_buyer in (buyerA,buyerB,buyerC):
        for record in the_buyer.in_all_rounds():
            if record.field_maybe_none("buyer_decision") == str(the_seller.role):
                score_list.append(record.score)
    if len(score_list) == 0:
        return None
    else:
...
def to_score_R2(the_buyer,var_of_all_buyers,avg_of_certain_buyers): # 输入buyerA,输出（buyerA，var,avg)
    score_list = []
    for i in the_buyer.in_all_rounds():  # 对于这个买家来说
        if i.field_maybe_none('score') != None: # 如果他轮轮的分数不是None
            score_list.append(i.field_maybe_none('score'))  # score_list列表中添加他的分数
    if len(score_list) == 0 or var(score_list) == 0:
        return the_buyer.field_maybe_none("score")  # 直接输出
    else:
        print("1",the_buyer.field_maybe_none('score'),"2",var_of_all_buyers,"3",var(score_list),"4",avg_of_certain_buyers,"5",mean(score_list),"6",sqrt(var_of_all_buyers/var(score_list)))
        return the_buyer.field_maybe_none('score')*sqrt(var_of_all_buyers/var(score_list))+avg_of_certain_buyers-mean(score_list)*sqrt(var_of_all_buyers/var(score_list))
...
class MyWaitPage(WaitPage):  # 计算seller的被购买次数，payoff，score_R1 每个人都会进入这个页面，但是以group为单位进行计算
    ...
    def after_all_players_arrive(group: Group):  # 当一个group同时到达，则开始计算seller的数据
        ...
        for the_seller in (sellerA, sellerB, sellerC):
            the_seller.average_score = average_score(the_seller,buyerA,buyerB,buyerC) # 生成seller字典{sellerA: 45.2}
            if the_seller.field_maybe_none('average_score') != None:
                seller_dic[str(the_seller.role)] = the_seller.average_score
        print("当前轮卖家平均分字典为：", seller_dic)
        ...
        if group.round_number == 1:  # 如果是第一轮，那么scoreR1就是score
            print("此时是第一轮，scoreR2=score")
            for the_buyer in (buyerA, buyerB, buyerC):
                the_buyer.score_R2 = the_buyer.field_maybe_none('score')  # 直接输出
        else:  # 如果是第二轮，第三轮，第四轮...
            for the_buyer in (buyerA,buyerB,buyerC):
                if the_buyer.field_maybe_none('score') == None:  # 如果当轮没有评分
                    the_buyer.score_R2 = None
                else:
                    all_buyers_score=[]
                    for i in (buyerA,buyerB,buyerC):
                    ...
                    if len(all_buyers_score) == 0 or var(all_buyers_score) == 0:
                        print("无买家评分或买家评分一样，所以此时scoreR2=score")
                        for the_buyer in (buyerA, buyerB, buyerC):
                            the_buyer.score_R2 = the_buyer.field_maybe_none('score')  # 直接输出
                    else:  # 到这里既不是第一轮，buyer又有打分，且[all buyer score list]有效
                        var_of_all_buyers = var(all_buyers_score)
                        # 每一轮和买家交易过的卖家的最新平均分的平均值
                        # 生成买家交易对象
                        for the_buyer in (buyerA, buyerB, buyerC):
                            trade_list=[]
                            trade_avgs_list=[]
                            for record in the_buyer.in_all_rounds():
                                if record.field_maybe_none('buyer_decision') != None:
                                    trade_list.append(record.buyer_decision)
                                    print("和该{}买家交易过的卖家是{}".format(str(the_buyer.role),trade_list))
                                    trade_avgs_list=list(map(lambda x: seller_dic[x],trade_list))
                                    print("平均分列表为",trade_avgs_list)
                            avg_of_certain_buyers = mean(trade_avgs_list)
                            if the_buyer.field_maybe_none('score') != None:
                                the_buyer.score_R2 = to_score_R2(the_buyer, var_of_all_buyers, avg_of_certain_buyers)
...
class BuyerScorePage(Page):
    ...
    def vars_for_template(player):
        ...
        player.average_score_R2 = target_seller.field_maybe_none("average_score_R2")  # 获取buyer做决策用的score_R
        htmltable = get_table(sellerA, sellerB, sellerC, player)
        return{
            "quality":player.quality,
            "price":player.price,
